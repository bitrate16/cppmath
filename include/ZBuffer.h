...
namespace spaint {
	...
	public:	
		...
		void triangle(const cppmath::vec3& v1_, const cppmath::vec3& v2_, const cppmath::vec3& v3_, const Color& c1, const Color& c2, const Color& c3) {
			...
			// XXX: Fix vertice sorting, add something based on http://www.sunshine2k.de/coding/java/TriangleRasterization/TriangleRasterization.html & vertice distance clipping
			...
			for (int x = 0; x <= (C.x - A.x); ++x) { // P
				...
				line(cppmath::vec3(A.x + x, ACy, ACz), cppmath::vec3(A.x + x, ABy, ABz), Color::interpolate(cA, cC, ACt), Color::interpolate(cA, cB, ABt));
			...
			for (int x = 0; x < (B.x - C.x); ++x) { // Q
				...
				line(cppmath::vec3(B.x - x, BAy, BAz), cppmath::vec3(B.x - x, BCy, BCz), Color::interpolate(cB, cA, BAt), Color::interpolate(cB, cC, BCt));
