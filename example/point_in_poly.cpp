...
// bash c.sh "-lX11" example/point_in_poly
...
class scene : public component {
	...
	void create() {
		get_paint().init_font();
		get_window().set_title("Point inside polygon example");
	};
			...
			if (points.size() > 1) {
				if (mouse2_down) {
					...
					for (int i = 1; i < points.size(); ++i)
						right = right < points[i].x ? points[i].x * 2.0 : right;
					
					// Ray coords
					vec2 ray_a = vec2(point.x, point.y);
					vec2 ray_b(right, point.y);
					int trace = 0;
					
					p.line(ray_a.x, ray_a.y, ray_b.x, ray_b.y);
					
					
					if (cppmath::math::inside_poly(vec2(point.x, point.y), points)) {
						
						p.color(255, 0, 255);
						
						XPoint verts[points.size()];
						for (int i = 0; i < points.size(); ++i) 
							verts[i] = { points[i].x, points[i].y };
						
						p.fill_poly(verts, points.size());
					}
				}
				...
				if (render_lines) {
					...
					if (points.size())
						p.line(points.front().x, points.front().y, points.back().x, points.back().y);
				}
