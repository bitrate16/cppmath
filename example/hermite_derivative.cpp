...
// Hermite spline derivative by points visualisation using spaint tool.

// bash c.sh "-lX11" example/hermite_derivative
...
class scene : public component {
	...
	void create() {
		get_paint().init_font();
		get_window().set_title("Hermite derivative example");
	};
	...
	void loop() {
		...
		if (resized || updated) {
			...
			if (points.size() > 1) {
				...
				vec2 old_point;
				double dt = 1.0 / (double) CURVE_STEPS;
				for (int i = 0; i < points.size(); ++i)
					for (double t = 0; t <= 1.0; t += dt) {
						vec2 point = hermite_derivative(points, derivatives, i, t);
						
						if (t != 0.0) 
							p.line(old_point.x, old_point.y, point.x, point.y);
