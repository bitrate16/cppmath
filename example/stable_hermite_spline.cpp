...
// Stable hermite spline spline by points visualisation using spaint tool.

// bash c.sh "-lX11" example/stable_hermite_spline
...
class scene : public component {
	...
	void create() {
		get_paint().init_font();
		get_window().set_title("Stable hermite spline example");
	};
	...
	void loop() {
		...
		if (!drag1_mode && mouse1_down) {
			...
			derivatives = stable_hermite_derivatives(points);
